# DK64 RetroAchievements — private Linux x86-64 smoke test
...
This is an **untested Linux development build for invited testers only**, not a public RetroAchievements release. It has passed build and offline checks under Ubuntu 24.04/WSL, but has **not** run in a native Linux game session or confirmed a Linux account award. Please do not repost it or describe it as certified. It uses the existing RA DK64 N64 set (game 10075); Hardcore and leaderboards are disabled.
...
- This particular companion was built on Ubuntu 24.04 and requires **glibc 2.38 or newer**, `GLIBCXX_3.4.30` or newer, `libcurl.so.4`, and `libSDL2-2.0.so.0`. A newer distro may still have other loader or game dependencies. Do **not** replace your system glibc to make this build load. If it fails to load, tell us your distro and error; we can build against a different baseline.
...
1. Close the game. Extract the ZIP. Optionally run `sha256sum -c SHA256SUMS.txt` in the extracted folder to check its four listed files. If you already use this mod, keep your old mod files somewhere outside `mods/` before replacing them.
